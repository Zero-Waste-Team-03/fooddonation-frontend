...
export function AppearanceSection({ initialValues, onSubmit }: AppearanceSectionProps) {
  const [activeTheme, setTheme] = useAtom(themeAtom);

  const form = useForm<AppearanceFormValues>({
    resolver: zodResolver(appearanceSchema),
    defaultValues: {
      ...initialValues,
      appearanceTheme: activeTheme === "dark" ? "FOREST_DARK" : "ETHOS_LIGHT",
    },
  });

  const handleSubmit = form.handleSubmit(async (data) => {
    setTheme(data.appearanceTheme === "FOREST_DARK" ? "dark" : "light");
    await onSubmit(data);
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <div className="grid grid-cols-1 gap-2 rounded-xl bg-muted p-1 sm:grid-cols-3">
                    <Button
                      type="button"
                      variant="ghost"
                      className={
