...
function DefaultLoader() {
  return (
    <div className="flex items-center gap-2 rounded-md border border-border bg-card px-3 py-2 text-sm text-muted-foreground shadow-card">
...
function DefaultMarkerIcon() {
  return (
    <span
      className="block h-3 w-3 rounded-full border-2 border-background bg-info shadow-card ring-2 ring-info/30"
      aria-hidden
    />
  );
}
  ...
  return createPortal(
    ...
    >
      {closeButton && (
        <button
          type="button"
          className="absolute right-2 top-2 inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground"
          aria-label="Close"
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "rounded-md border border-border bg-card px-2 py-1 text-xs text-card-foreground shadow-card",
        className,
      )}
    >
      {children}
...
function MarkerLabel({
  ...
  return (
    <div
      className={cn(
        "absolute left-1/2 z-10 -translate-x-1/2 whitespace-nowrap text-xs font-medium text-foreground",
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col overflow-hidden rounded-md border border-border bg-background shadow-card">
      {children}
    </div>
  );
}
...
function ControlButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="inline-flex h-9 w-9 items-center justify-center border-b border-border bg-background text-foreground last:border-b-0 hover:bg-muted disabled:opacity-50"
    >
      {children}
    </button>
  );
...
function CompassButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <ControlGroup>
      <ControlButton onClick={onClick} label="Reset compass">
        <span
          ref={compassRef}
          className="inline-flex h-6 w-6 items-center justify-center transition-transform"
          aria-hidden
        >
          <svg viewBox="0 0 24 24" className="h-5 w-5 text-primary">
            <path
              fill="currentColor"
              d="M12 2l3 8H9l3-8zm0 20l-3-8h6l-3 8z"
            />
          </svg>
        </span>
      </ControlButton>
...
type MapPopupProps = Omit<PopupOptions, "className" | "closeButton"> & {
  longitude: number;
  latitude: number;
  onClose?: () => void;
  children: ReactNode;
...
function MapClusterLayer({
  ...
  useEffect(() => {
    ...
    const handlePointClick = (
      ...
      onPointClick(feature as unknown as GeoJSON.Feature, coordinates);
    };
